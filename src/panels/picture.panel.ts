...
function retrievePhoto(email: string) {
    if (!email || email == "") return blankPhoto;

    // Replace with your company outlook server
    return `https://mail.example.net/owa/service.svc/s/GetPersonaPhoto?size=HR96X96&email=${email}`;
}
